...
if (os.path.isfile(filename)):
  ...
  lines += ax1.plot(Nv, spatiocyte_point_data,'r+', fillstyle='none', markersize=markersize, mew=mew, label=r'Spatiocyte ($V=3\ \mathrm{\mu m}^{3}$)')
  ax1.annotate(r'$T\propto N$', xy=(X[9], spatiocyte_point_data[9][0]),  xycoords='data', xytext=(-16, -31), textcoords='offset points', color='r', size=legendFontSize)
...
if (os.path.isfile(filename)):
  ...
  lines += ax1.plot(Nv, spatiocyte_point_dilute_data,'rD', fillstyle='none', markersize=markersize, mew=mew, label=r'Spatiocyte ($V=30\ \mathrm{\mu m}^{3}$)')
  ax1.loglog(X, 0.33*X, 'r', linewidth=1.5)

filename = path+'smoldyn/smoldyn_out.py'
if (os.path.isfile(filename)):
  imp.load_source('smoldyn_out', filename)
  from smoldyn_out import *
  lines += ax1.plot(Nv, smoldyn_data,'gx', fillstyle='none', markersize=markersize, mew=mew, label=r'Smoldyn ($V=3\ \mathrm{\mu m}^{3}$)')

filename = path+'smoldyn/smoldyn_dilute_out.py'
if (os.path.isfile(filename)):
  imp.load_source('smoldyn_dilute_out', filename)
  from smoldyn_dilute_out import *
  lines += ax1.plot(Nv, smoldyn_dilute_data,'gs', fillstyle='none', markersize=markersize, mew=mew, label=r'Smoldyn ($V=30\ \mathrm{\mu m}^{3}$)')

filename = path+'fastbd/fastbd_out.py'
if (os.path.isfile(filename)):
  imp.load_source('fastbd_out', filename)
  from fastbd_out import *
  lines += ax1.plot(Nv, fastbd_data,'b1', fillstyle='none', markersize=markersize, mew=mew, label=r'Fast BD ($V=3\ \mathrm{\mu m}^{3}$)')

filename = path+'fastbd/fastbd_dilute_out.py'
if (os.path.isfile(filename)):
  imp.load_source('fastbd_dilute_out', filename)
  from fastbd_dilute_out import *
  lines += ax1.plot(Nv, fastbd_dilute_data,'bo', fillstyle='none', markersize=markersize, mew=mew, label=r'Fast BD ($V=30\ \mathrm{\mu m}^{3}$)')
...
ax1.tick_params(axis='y',which='both',direction='in',length=0,width=2)
...
plt.savefig('diffusion_point.pdf', format='pdf', dpi=800)# bbox_inches='tight', pad_inches=0)
plt.show()
