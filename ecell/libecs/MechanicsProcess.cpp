  ...
  for (int i(0);i<ns;i++)
    {
	  ...
	  realHvec[i][j][0]=(realHvec[i][j][0]*minRatio+translate)/
                            (2*voxelRadius);
	  realHvec[i][j][1] = hvec[i][j][1]+(-minhvecY);
	  realHvec[i][j][1]=(realHvec[i][j][1]*minRatio+translate)/ 
                            (2*voxelRadius);
	  realHvec[i][j][2] = hvec[i][j][2]+(-minhvecZ);
	  realHvec[i][j][2]=(realHvec[i][j][2]*minRatio+translate)/
                            (2*voxelRadius);
	}
    }
}
...
void MechanicsProcess::getBLTR(std::vector<Point>& newNode)
{
  ...
  for(unsigned i(0);i<4;i++)
    {
      theSpatiocyteStepper->point2global(newNode[i], row[i], col[i], lay
                                         [i]);
  ...
  topRight.z = maximumZ+(normVoxelRadius+theSpatiocyteStepper
                         ->getRowLength());
  if(topRight.x>lengthX)
    {
      topRight.x=lengthX;
    }
  if(topRight.y>lengthY)
    {
      topRight.y=lengthY;
    }
  if(topRight.z>lengthZ)
    {
      topRight.z=lengthZ;
    }
  if(bottomLeft.x<0)
...
void MechanicsProcess::calculateSurfaceNormal(Point& node1, Point& node2, 
Point& node3, Point& fixsurfaceNormal, double& fixsurfaceDisplace)
{
  Point AB;
  Point AC;
  AB = sub(node2,node1);
...
bool MechanicsProcess::isOnBelowSideSurface(Point& node1, Point& node2,  
...
bool MechanicsProcess::isOnAboveSurface(Point& aPoint, Point& 
